  ...
  const handleNewPassword = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/auth/change-password", {
        ...
        body: JSON.stringify({ email, session: pendingSession, newPassword, role: "admin" }),
      });
      if (!res.ok) {
      ...
      <div className="w-full lg:w-1/4 flex items-center justify-center p-8 bg-background">
        <div className="w-full max-w-sm">
          ...
          <div className="mt-5 text-center">
            <Link href="/login/agent" className="text-sm underline underline-offset-4 text-muted-foreground hover:text-primary">
